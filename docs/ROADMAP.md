...
1. **Duplicate-submission detection.** ✅ **SHIPPED 2026-07-20.** Two-stage: 256-bit dHash prefilter + pixel-difference confirm (64-bit and even 1024-bit dHash alone collapse different same-type synthetic forms to distance 0 — measured, not assumed). Flags at intake with `duplicate_of`, side-by-side compare in Review, "Keep — not a duplicate" / discard, never auto-drops. Zero-byte/unreadable uploads now 400 (closes IMPROVEMENTS #14). Pre-feature/seed docs lazily migrate into detection at next intake. Honest limit (docs/API.md): catches re-drops/re-encodes/downscales; a scan-vs-phone-photo pair is NOT caught (unaligned pixels) — do not demo that pair as a catch.
2. **Cross-document anomaly flags.** Within a client's bin: SSN/TIN mismatch across docs, employer-name drift, tax-year mismatch, amount outliers vs prior docs. Flags surface in Review with a one-line model explanation ("why I flagged this"). Gate: confirm with a judge/user firsthand that this is the missing "interesting AI" before going deep — the dad demo Q&A (docs/AI-NATIVE-FIRM.md §7) is the designated gate check.
3. **Watched intake folder (T70).** ✅ **SHIPPED 2026-07-20.** `KEEPBOOK_INBOX` env (unset = off), stdlib 2s polling watcher, (size,mtime)-stable-before-ingest, content-sha256 no-reingest across restarts, originals never moved/deleted, same `/intake` code path (validation + dup-flag), `source:"folder"` chip in Review, `/health` reports the inbox. Images only for now — PDFs in the inbox are ignored (needs an ambient-password story first; see API.md note).
4. **Auto-drafted "still waiting on" nudges.** ✅ **SHIPPED 2026-07-20.** `GET /clients/{id}/nudge`: model drafts via new text-only `generate_text` adapter call, deterministic post-checks (client name + every missing doc verbatim, no placeholders, ≤900 chars) with template fallback — never 500s on model misbehavior. Copy-only modal on gapped client cards ("drafted by the model — review before sending"); no send capability anywhere.
...
9. **PDF ingestion, incl. password-protected PDFs** ✅ **SHIPPED 2026-07-20** (promoted — IMPROVEMENTS #6): `/intake` accepts `.pdf` (magic-number detected), pypdfium2 (BSD/Apache — not AGPL PyMuPDF) renders each page to ~200-DPI PNG server-side, one document per page with `page_number` + `source_file`, 20-page cap, dup-flagging on rendered pages. Encrypted PDFs: optional `password` field, `password_required:`/`password_incorrect:` machine-checkable 400s, frontend prompts + retries; password lives in memory only — grep-proven absent from state/events/raws/logs. Known follow-up: fillable AcroForm PDFs would render with blank field values (needs `init_forms()` render path); static-text bank statements unaffected.
